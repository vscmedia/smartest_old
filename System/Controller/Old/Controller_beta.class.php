...
if(!is_file($include_path."PEAR.php") && !is_file($include_path."XML/Unserializer.php")){
  // ini_set('include_path','.:Libraries/pear/:Libraries/Smarty-2.6.9/:Libraries/Smarty-2.6.13/');
  // echo $include_path."PEAR.php";
}
require_once 'PEAR.php';
require_once 'XML/Unserializer.php'; 
...
class Controller{
 ...
 /**
  ...
  function Controller($filename = 'controller.xml'){
    ...
    if(isset($htcontent) && preg_match('/RewriteEngine\s+on/i',$htcontent) ){
      ...
      $this->debugInfo = ".htaccess was not found, but was succesfully written";
    ...
    if(isset($data['modules']) && preg_match ( '/\/$/', $data['modules'])){      
      if(preg_match ( '/^\//', $data['modules'])){
        if(is_dir($data['modules'])){
          ...
          die("The absolute path to the 'Models' directory, defined in controller.xml, does not exist");
        }      
      }
      else{
        if(is_dir(getcwd()."/".$data['modules'])){
          $this->modules = getcwd()."/".$data['modules'];
        }
        else{
          die("The relative path to the 'Models' directory, defined in controller.xml, does not exist");
        }
      }
    }
    else{
    ...
    if(isset($data['module']) ){
      ...
      die("&lt;module&gt; not defined in controller");
    }
  ...
  */
  function performAction(){
    ...
    if( $this->isPrivilegedUser() && !(isset($_POST['username']) && isset($_POST['password']) )){
      ...
      $classFile = $this->modules.$className.".class.php";
      if(!file_exists($classFile)){
        die("$className.class.php not found in ".$this->modules);      
      }      
      ...
      if( method_exists( $page, $webmethod  )){
        ...
        die("The specified method is not implemented in this class");        
      }      
    }
    // The user is not privilaged, but they are trying to login
      ...
      require_once($this->modules.$className.".class.php");
      ...
      if( method_exists( $page, $authMethod  )){
        ...
        die("Authentication class is required");
      }
      ...
      die("A section needs to be defined on successful login");
  ...
  */ 
  function getAlias($sectionName){		
    ...
    if(isset($this->aliases)){
      ...
      if(strlen($this->aliases[$request])){
  ...
  */
  ...
  function _debug(){
    ...
    <div id=\"debug_window\" style=\"opacity: .99; z-index:100; padding-left: 10px; padding-top: 10px; position: fixed; font-size: 10px; width: 600px; bottom: 0px; right: 0px; background-color:#ff6\">
    ...
    if($this->debugLevel > 0){
      ...
      echo "Models Path = ".$this->modules;
      echo (is_dir($this->modules)) ? "\n" : " (Pages not found)\n";

...
class Controller{
	...
	function Controller($filename = "controller.xml", $log="", $automatic=true, $use_cache=true){
		...
		if(isset($_SESSION["xmlDataHash"]) && $_SESSION["xmlDataArray"]){ // If the xml controller file is already cached in the session
			...
			if($_SESSION["xmlDataHash"] == md5_file($filename)){
				...
				$this->_log("Controller XML file unchanged. Loaded controller data from session.");
				$this->xmlDataArray = $_SESSION["xmlDataArray"];
				
			}else{
				if($_SESSION["xmlDataArray"] = $this->loadXmlControllerFile($filename)){
					$this->_log("Controller XML file has been modified.");
					...
					// file not loaded
					// ERROR
					$this->_error("Controller XML file was not found or could not be loaded: $filename");
				}
			}
		}else{ // try and load the xml controller file
		
			// LOG: XML Data Not Cached
			$this->_log("Controller XML data not found in session cache");
			if($_SESSION["xmlDataArray"] = $this->loadXmlControllerFile($filename)){
				$this->_log("Loaded XML data From file: $filename");
	...
	function loadXmlControllerFile($filename){
		...
		$this->_log("Attempting to load principal controller XML file: $filename");
	...
	function getControllerUrlRequest(){
		...
		if(@strlen($this->xmlDataArray['domain-path']) > 1){
			
			$domain_path = $this->xmlDataArray['domain-path'];
	...
	function getModuleDirectories(){
		if($this->xmlDataArray['modules']){
			$directories = explode(":", $this->xmlDataArray['modules']);
	...
	function getModules(){
		...
		if(is_array($this->xmlDataArray['module'])){
			
			if(isset($this->xmlDataArray['module']['name'])){
				$modules = array();
				$modules[0] = $this->xmlDataArray['module'];
			}else{
				$modules = $this->xmlDataArray['module'];
			}
			...
			foreach($modules as $key=>$module){
				...
				if(isset($modules[$key]['ajax']) && strlen($modules[$key]['ajax']) > 0){

...
SmartestFileSystemHelper::include_group(
	...
	'System/Data/SmartestDataSet.class.php',
...
class SmartestResponse{
	...
	private $publicMethodNames = array("renderPageFromUrl", "renderPageFromId", "doAuth");
	...
	function build(){
		...
		// $this->controller->addProperty('smarty', $this->smarty);
		// var_dump(in_array($this->controller->getMethodName(), $this->publicMethodNames));
		// var_dump($this->isSystemClass());
		
		// check that key directories are writable
		// $this->checkWritableDirPermissions();
		...
		/* if(isset($_SESSION['user']) && is_array($_SESSION['user'])){
			...
			$this->smarty->assign("_user", array());
		} */
		
		// $this->controller->addProperty('_user', $_SESSION['user']);
		...
		if($this->getIsWebsitePage($this->url)){
			
			// Request is for a cms page
			define("SM_CONTROLLER_SECTION", "website");
			define("SM_CONTROLLER_MODULE", "website");
			define("SM_CONTROLLER_MODULE_DIR", SM_ROOT_DIR.$this->controller->getModuleDirectory());
			define("SM_CONTROLLER_METHOD", "renderPageFromUrl");
			define("SM_CONTROLLER_DOMAIN", $this->controller->getDomain());
			define("SM_CONTROLLER_TEMPLATE_FAMILY", "Website");
			define("SM_CONTROLLER_TEMPLATE_FILE", "System/Website/_default.tpl");
			
		}else{
		
			// Request is for another app
	...
	static function getGetVariables(){
		return $this->controller->getGetVariables();
	}
	
	function getIsWebsitePage($url){
	    if(!isset($this->isWebsitePage)){
	        if(SmartestCache::hasData('request_is_website_page_'.md5($url), true)){
                $this->isWebsitePage = SmartestCache::load('request_is_website_page_'.md5($url), true);
            }else{
		        $sql = "SELECT * FROM Pages, PageUrls WHERE PageUrls.pageurl_page_id=Pages.page_id AND PageUrls.pageurl_url='$url'";
		        $this->isWebsitePage = (bool) $this->database->howMany($sql);
		        SmartestCache::save('request_is_website_page_'.md5($url), $this->isWebsitePage, -1, true);
	        }
	    }
	    
	    return $this->isWebsitePage;
	}
	
	function checkRequiredExtensionsLoaded(){
...
';
	...
	public function getFlatHtmlFileName($string){
	...
	public function fetch(){
		...
		if(SM_CONTROLLER_METHOD == "renderPageFromUrl" || SM_CONTROLLER_METHOD == "renderPageFromId"){
			...
			if(defined("SM_PAGE_ID")){	
			
				$filename = $this->getFlatHtmlFileName(SM_CONTROLLER_URL);
				
				if(file_exists($filename) && SM_PAGE_CACHE_AS_HTML == "TRUE"){	
	...
	public function finish(){
		...
		// print_r($_SESSION);
		// $this->authentication->checkLoginDetails('marcus', 'london!smartest');
		// print_r($_SESSION);
		...
		// echo $this->controller->getNamespace();

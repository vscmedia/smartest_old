...
class SmartestRgbColor implements ArrayAccess, SmartestBasicType, SmartestStorableValue, SmartestSubmittableValue{
    ...
    public function setValue($v){
        ...
        if(preg_match(self::RGB_HEX_MATCH, $v, $matches)){
            ...
            $red_h = ($this->_depth == 8) ? substr($code, 0, 1).substr($code, 0, 1) : substr($code, 0, 2);
            $green_h = ($this->_depth == 8) ? substr($code, 1, 1).substr($code, 1, 1) : substr($code, 2, 2);
            $blue_h = ($this->_depth == 8) ? substr($code, 2, 1).substr($code, 2, 1) : substr($code, 4, 2);

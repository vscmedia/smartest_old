...
class SmartestMysql{
	...
	protected function getSelectQueryResult($query, $refresh=false){
	    ...
	    // if(in_array($hash, $this->queryHashes) && !$refresh){
	    if(SmartestCache::hasData($cache_name, true)){
	        return $this->loadQueryDataFromCache($hash);
	    }else{
    		...
    		$this->saveQueryDataToCache($hash, $resultArray);
	...
	public function queryToArray($querystring){
		...
		if(!$this->dblink && !$this->reconnect()){
		    ...
		    if(in_array($this->getQueryType($querystring), $this->retrievalQueryTypes)){
		    
		        return $this->getSelectQueryResult($querystring);
	...
	protected function recordQuery($querystring){
		...
		$hash = $this->getHashFromQuery($querystring);
	    
	    if(in_array($hash, $this->queryHashes)){
	...
	protected function loadQueryDataFromCache($query){
	    ...
	    if(SmartestCache::hasData($cache_name, true)){
	        return SmartestCache::load($cache_name, true);
	    }else{
	...
	public function clearQueryHistoryCache(){
	    ...
	    foreach($this->queryHashes as $hash){
	        $cache_name = 'smartest_mysql_cached_result'.$hash;

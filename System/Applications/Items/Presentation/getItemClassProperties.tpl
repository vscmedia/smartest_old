...
<ul class="tree-parent-node-open" id="tree-root">
  {defun name="menurecursion" list=$definition}
    ...
    <li >
       <a id="item_{$element.id}" class="option" href="javascript:nothing()" onclick="setSelectedItem('{$element.id}','{$element.itemproperty_varname}', 'fff');" ondblclick="window.location='{$domain}{$section}/editItemProperty?class_id={$itemclass.itemclass_id}&amp;itemproperty_id={$element.itemproperty_id}'">		 
